...
def is_record(game_grid):
    ...
    while True:
        try:
            result = get_rank()
            global list_name_amazon
            global list_score_amazon
            list_name_amazon = [str(result[0][1]),str(result[0][3]), str(result[0][5])]
            list_score_amazon = [int(result[0][2]), int(result[0][4]), int(result[0][6])]
            list_name = list_name_amazon
            list_score = list_score_amazon
            print(result)
...
def save_record(name, score, pos):
    ...
    if amazon:
        ...
        global list_score_amazon
        d = shelve.open(record_path)
        list_name_amazon.insert(pos, name)
        list_score_amazon.insert(pos, score)
        list_score_amazon.pop()
        list_name_amazon.pop()
        d['name'] = list_name_amazon
        d['score'] = list_score_amazon
        d.close()
        
        mydb = mysql.connector.connect(host="dbcw.cy6ap9vbmbq6.us-east-2.rds.amazonaws.com", username="admin", password="cw12345678", database="basecw")
        mycursor = mydb.cursor()
        query = "INSERT INTO record_3 (name_1, score_1,name_2,score_2,name_3,score_3) VALUES ((%s),(%s), (%s),(%s),(%s),(%s))"
        values = (str(list_name_amazon[0]),int(list_score_amazon[0]), str(list_name_amazon[1]),int(list_score_amazon[1]),str(list_name_amazon[2]),int(list_score_amazon[2]))
...
def get_rank():
    mydb = mysql.connector.connect(host="dbcw.cy6ap9vbmbq6.us-east-2.rds.amazonaws.com", username="admin", password="cw12345678", database="basecw")
    mycursor = mydb.cursor()
    query = "SELECT * FROM record_3 WHERE record_id=(SELECT max(record_id) FROM record_3)"
...
mycursor = mydb.cursor()
query = "INSERT INTO record_3 (name_1, score_1,name_2,score_2,name_3,score_3) VALUES ((%s),(%s), (%s),(%s),(%s),(%s))"
values = (str(list_name[0]),int(list_score[0]), str(list_name[1]),int(list_score[1]),str(list_name[2]),int(list_score[2]))
mycursor.execute(query, values)
mydb.commit()
mydb.close()
'''